...
var sparql_endpoint = window.location.protocol + '//' + window.location.host + '/sparql-2/annotations/filter';
  ...
  request
    ...
    .end((err, res) => {
      if (!err) {
        ...
        document.getElementById("resultsms").innerHTML = "Queried all annotations in " + Math.round(elapsed) + " ms.";
      }
...
function displayResults(results) {
  ...
  if (bindings === null || bindings.length == 0) {
    ...
    for (let i = 0; i < bindings.length; i++) {
      ...
      resultString += (i+1) + ') ' + result.annotation.value + ' --> ' + result.text.value + '\n';
    }
  }
