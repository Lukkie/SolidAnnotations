...
var sparql_onegraph_endpoint =
  window.location.protocol + "//" + window.location.host + "/sparql-2";
...
function randomUser() {
  return new Promise(function(resolve, reject) {
    let data = {};
    try {
      data.firstName = Math.random()
        .toString(36)
        .substring(7);
      data.lastName = Math.random()
        .toString(36)
        .substring(7);
      resolve(data);
    } catch (err) {
...
function generateAnnotations(
  ...
  return new Promise(function(resolve, reject) {
    ...
    for (let i = 0; i < number_of_annotations; i++) {
      ...
      let upper_comment_bound =
        lower_comment_bound +
        Math.ceil(Math.random() * (comment.length - lower_comment_bound));
      comment = content
        .split(" ")
        .slice(lower_comment_bound, upper_comment_bound)
        .join(" ");

      let username =
        user.name || user.firstName + " " + user.lastName || "Username";
      ...
      solid.web
        ...
        .then(function(meta) {
          ...
          if (annotations.length == number_of_annotations) {
            ...
            console.log(
              "-- [SOLID] Annotations created in " + elapsed + " ms --"
            );
            let average_time = elapsed / number_of_annotations;
            console.log(
              "-- [SOLID] Average storage time for " +
                number_of_annotations +
                " annotations is " +
                average_time +
                " ms --"
      ...
      request
        ...
        .then(function(res) {
          ...
          console.log(
            "(" +
              i +
              ") Multiple graphs -- Succesfully created graph of " +
              annotation +
              " ( " +
              endpoint +
              "/" +
              annotation_split[annotation_split.length - 2] +
              "/" +
          ...
          if (counter == number_of_annotations) {
            ...
            elapsed = 1000 * elapsedS + elapsedMs;
            console.log(
              "-- [SPARQL] Annotations created in " + elapsed + " ms --"
            );
            let average_time = elapsed / number_of_annotations;
            console.log(
              "-- [SPARQL] Average storage time for " +
                number_of_annotations +
                " annotations is " +
                average_time +
                " ms --"
      ...
      request
        ...
        .then(function(res) {
          ...
          console.log(
            "(" +
              i +
              ") SPARQL single graph -- Succesfully stored annotation with identifier " +
              url +
              " to the graph (" +
              elapsed +
              " ms)"
...
function loadSPARQLAnnotations(annotations) {
  return new Promise(function(resolve, reject) {
    ...
    for (let i = 0; i < annotations.length; i++) {
      ...
      let individual_annotation_timer = hrtime();
      request.get(annotation_url).end((err, res) => {
        if (!err) {
          let elapsedS = hrtime(individual_annotation_timer)[0];
          let elapsedMs = hrtime(individual_annotation_timer)[1] / 1e6;
          let elapsed = 1000 * elapsedS + elapsedMs;
...
function initialize() {
  solid
    ...
    .then(function(profile) {
      ...
      var annotationService_location = profile.find(
        vocab.oa("annotationService")
      );
      if (!annotationService_location) {
        ...
        var toIns = [
          rdf.triple(
            ...
            vocab.oa("annotationService"),
            rdf.sym(profile.storage + "/annotations")
          )
        ];
        solid.web
          .patch(profile.baseProfileUrl, toDel, toIns)
          .then(function(response) {
      ...
      solid.web
        .get(profile.storage + "/annotations")
        ...
        .catch(function(err) {
          ...
          var data = rdf.triple(
            rdf.sym(profile.storage + "/annotations"),
            vocab.ldp("constrainedBy"),
            rdf.sym("http://www.w3.org/TR/annotation-protocol/")
          );
          solid.web
            .createContainer(profile.storage, "annotations", {}, data)
...
function toggleNumberOfUsers(checked) {
  var usersParameterSelection = document.getElementById(
    "usersParameterSelection"
  );
